...
export function CyberDefense({ isGenerating = false, startTime = Date.now() }: CyberDefenseProps) {
            ...
            if (enemy.health <= 0) {
              ...
              if (bullet.pierce && !bullet.hasRicocheted && enemiesRef.current.length > 1) {
                ...
                if (nearestEnemy !== null) {
                  const angle = Math.atan2((nearestEnemy as Enemy).y - enemy.y, (nearestEnemy as Enemy).x - enemy.x);
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      if (container) {
        ...
        const targetWidth = isMobileView ? 400 : 800;
        const targetHeight = isMobileView ? 500 : 600;
        
        // Calculate actual size based on container
        const maxWidth = Math.min(containerRect.width * 0.9, targetWidth);
        const maxHeight = Math.min(containerRect.height * 0.84, targetHeight);
        
        const finalWidth = Math.max(400, Math.min(maxWidth, targetWidth));
        const finalHeight = Math.max(500, Math.min(maxHeight, targetHeight));
        ...
        // Center player
        playerRef.current.x = canvas.width / (pixelRatio > 1 && !isMobileView ? pixelRatio : 1) / 2;
        playerRef.current.y = canvas.height / (pixelRatio > 1 && !isMobileView ? pixelRatio : 1) / 2;
      }
    };
  ...
  return (
    <div className="w-full h-full max-w-7xl mx-auto">
      <div className="h-full flex flex-col bg-cyber-dark/90 backdrop-blur-sm rounded-lg border border-cyber-cyan/20 p-2 sm:p-4 max-h-[96vh]">
        ...
        <div className="relative flex-1 flex items-center justify-center max-h-[72vh]">
          <canvas
            ref={canvasRef}
            className={`bg-cyber-black rounded touch-none select-none w-full h-full ${
